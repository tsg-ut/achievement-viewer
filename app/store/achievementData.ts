...
const localMutations = {
	setList(state: State, list: AchievementData[]) {
		state.list = list;
...
const localGetters = {
	...
	getById: (state: State) => (id: string) => {
		...
		return datum ?? ({id} as AchievementData);
	},
	getByCounter: (state: State) => (counter: string) =>
		state.list
			.filter((datum) => datum.counter === counter)
			.sort((a, b) => a.value - b.value),
	getByCategory: (state: State) => (category: string) =>
		state.list
